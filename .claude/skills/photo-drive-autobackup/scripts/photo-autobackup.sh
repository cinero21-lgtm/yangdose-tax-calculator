...
#
# 다만 앱이 내보낸 전사는 시각 뒤에 꼬리표를 하나 붙인다. 실측:
#   녹음   통화 01075907672_260820_181242.m4a
#   전사   통화 01075907672_260820_181242_original.txt
# 그래서 끝 고정은 유지하되 '숫자가 없는 꼬리표' 하나까지만 허용한다. 꼬리표에
# 숫자를 허용하면 위의 전화번호 오인이 그대로 되살아난다('_112710'이 꼬리표가
# 되어 '112222_260820'이 시각으로 뽑힌다). 그래서 [^0-9]+ 다.
stamp_of() {
  local base="${1##*/}"
  printf '%s' "${base%.*}" | sed -nE 's/.*([0-9]{6}_[0-9]{6})(_[^0-9]+)?$/\1/p'
}
