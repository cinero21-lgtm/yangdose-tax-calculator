...
2. **찍은 지 `HOLD_DAYS`(기본 7일)가 안 된 것은 뺀다** — 계획 단계에서 미리 제외하므로
   계획에 적힌 건수가 곧 실제로 옮기는 건수다. 최근 사진까지 한 번에 비우려면
   `HOLD_DAYS=0 photo-autobackup.sh migrate`
3. **무엇을 옮길지 계획을 먼저 보여 준다** — 건수, 총 용량, 폴더별 내역, 드라이브 여유 공간
4. 사용자가 `yes`를 입력해야 진행한다
5. 파일마다: 업로드 → 원격 MD5와 로컬 MD5 대조 → 일치할 때만 휴지통으로 이동
6. 끝나면 폰에 사진이 남았는지 자동으로 재확인한다(유예 중인 것은 "남았다"로 세지 않는다)
