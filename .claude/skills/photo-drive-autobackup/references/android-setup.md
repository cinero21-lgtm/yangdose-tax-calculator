...
구글 콘솔에서 만드는 것은 브라우저 작업이고, 폰에 심는 것은 명령 한 줄이다.

1. https://console.cloud.google.com — **드라이브 백업에 쓰는 그 구글 계정**으로
   로그인 → 프로젝트 만들기 (이름 아무거나, 예: `rclone-backup`)
2. API 및 서비스 → 라이브러리 → **Google Drive API** 검색 → **사용 설정**
3. OAuth 동의 화면 → User Type **외부(External)** → 앱 이름 `rclone`,
   이메일 두 칸에 본인 메일 → 나머지는 기본값으로 저장
4. **앱 게시(프로덕션 전환)를 반드시 누른다.** '테스트' 상태로 두면 인증이
   **7일마다 끊겨** 백업이 매주 조용히 멈춘다. '확인되지 않은 앱' 상태라도
   본인만 쓰는 앱이므로 게시해도 된다
5. 사용자 인증 정보 → 사용자 인증 정보 만들기 → **OAuth 클라이언트 ID** →
   유형 **데스크톱 앱** → 만들기 → **클라이언트 ID** 와 **보안 비밀** 두 값을 복사

폰(Termux)에서:

```sh
photo-autobackup.sh client-id '<클라이언트ID>' '<보안비밀>'
```

값의 꼴을 검사하고(뒤바꿔 넣는 실수 방지), rclone 설정에 심은 뒤(.bak 백업)
재인증 브라우저를 띄우고, 드라이브 접속까지 확인해 준다.
