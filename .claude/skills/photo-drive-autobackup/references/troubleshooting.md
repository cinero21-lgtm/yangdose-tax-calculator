...
## 앱에서 내보낸 전사가 녹음에 안 붙는다

`photo-autobackup.sh transcripts` 를 돌린다. 아무것도 안 바꾸고, 무엇이 남았는지
먼저 보여 준다.

- **`[짝지음]` 이 안 뜨고 `[건너뜀] ... 시각이 없다`** → 파일명 **끝**에
  `_YYMMDD_HHMMSS` 가 없다. 삼성 노트를 거치면 노트 제목이 이름이 되어 이렇게
  된다. 화면에 같이 찍히는 `transcripts pair` 명령을 그대로 쓰되, 아래
  '아직 전사가 없는 녹음' 목록에서 시각을 골라 넣는다.
- **파일이 아예 안 보인다** → 기본으로 `Download`·`Documents` 만 뒤진다.
  다른 폴더에 떨어졌으면 설정에 `TRANSCRIPT_DROP_DIRS` 로 적는다.
- **`[이미있음]`** → 이미 붙어 있다. 다시 붙이지 않는 게 맞다.

짝이 지어진 뒤 올리는 것은 `calls` 다. 한 번에 하려면
`photo-autobackup.sh transcripts --upload`.
